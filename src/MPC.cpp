  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t t = 1; t < N; t++) {
      ...
      for (int i = 1; i < coeffs.size(); i++) {
        arch += coeffs[i] * i * CppAD::pow(x0, i-1);
      }

      //AD<double> f0 = result;

      AD<double> psides0 = CppAD::atan(arch);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs,vector<double> & mpc_xs, vector<double> & mpc_ys) {
// CppAD::ipopt::solve_result<Dvector> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  bool ok = true;
  size_t i;
  ...
  // upper and lower limit for x,y,psi,v,cte and psi erro
  for (i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -1.0e10;
    vars_upperbound[i] = 1.0e10;
  }

  // set angle from -25 to +25 degree or -0.436 rad to +0.436 rad
  ...
  for(size_t ii=0;ii<N;ii++){
    val = solution.x[x_start + ii];
    //std::cout << val << ", ";
    mpc_xs.push_back(val);
    //mpc_y.push_back(solution.x[y_start + ii]);
  }
  ...
  //std::cout << "MPC y : ";
  for(size_t ii=0;ii<N;ii++){
    val = solution.x[y_start + ii];
    //std::cout << val << ", ";
    mpc_ys.push_back(val);
    //mpc_y.push_back(solution.x[y_start + ii]);
  }
